...
void Decoder::repairOperator(std::pair < std::vector < double >, std::vector < double > > &chromosome, std::vector < std::pair < double, int > > &ranking, std::vector < int > &preSelectedItems, int preSelectedItemsSize, std::vector < int > &weight, std::vector < int > &profit, int capacity) const { // clog << "repairOperator1" << endl;
    ...
    totalWeight = 0;
    for(int k = 0; k < preSelectedItemsSize; ++k) {        
...
void Decoder::decode(std::pair < std::vector < double >, std::vector < double > > &chromosome) {
    ...
    int idItem = 1;
    int totalWeight = 0;
    std::vector < int > selectedItems(m);
    int selectedItemsSize = 0;
    for(int i = n - 1; i < n + m - 1; ++i) {
        if(chromosome.first[i] - EPS >= alpha) {            
            ...
            totalWeight += Data::getInstance().items[idItem].weight;
            selectedItems[selectedItemsSize++] = idItem;                        
        }
        idItem += 1;
    }
    ...
    if(totalWeight > Data::getInstance().capacityOfKnapsack) {
        repairOperator(chromosome, ranking, selectedItems, selectedItemsSize, weight, profit, Data::getInstance().capacityOfKnapsack);
    }    
...
void Decoder::localSearch(std::pair < std::vector < double >, std::vector < double > > &chromosome, Population* allpop) {
    ...
    for(int i = tour.size() - 1; i >= 0; --i) {
        int currCity = tour[i];
        if(totalWeight + weight[currCity] <= Data::getInstance().capacityOfKnapsack + EPS) totalWeight += weight[currCity];
        else weight[currCity] = profit[currCity] = 0;
    }   
    ...
    std::uniform_int_distribution < int > distribution1(1, (int)tour.size()-2);

    for(int _i = 0; _i < 100; ++_i) { 
        
        double prevTime = totalTime;
        
        int a, b;
        do {
            a = distribution1(generator);
            b = distribution1(generator);
        } while (a == b); // || abs(a - b) > (int)tour.size()*0.1);
        
        if(a > b) std::swap(a, b);
        
        std::reverse(tour.begin() + a, tour.begin() + b);
...
void Decoder::save(const std::pair < std::vector < double >, std::vector < double > > &chromosome, ofstream &foutX, ofstream &foutF) const {
    ...
    foutF << fixed << setprecision(12) << totalTime << ' ' << fixed << setprecision(0) << totalProfit << endl;
}
